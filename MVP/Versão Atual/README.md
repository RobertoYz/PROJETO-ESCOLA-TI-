...
4. Configure as variáveis de banco de dados para usar o MySQL do XAMPP com as credenciais padrão:
...
5. Configure a chave da API do gateway de pagamentos:

    ABACATEPAY_API_KEY=sua_chave_de_teste_aqui
...
### Passo 4: Construir as Tabelas (Migrations)
...
### Passo 5: Popular o Banco de Dados (Seeders)
Para que o fluxo de registro e precificação do SaaS funcione corretamente, é obrigatório popular a tabela com os planos padrões. Execute o comando:

    php artisan db:seed --class=PlanoSeeder

> **Dica:** Caso queira recriar o banco do zero e já popular as tabelas em um único comando no futuro, utilize: `php artisan migrate:fresh --seed`
...
**Observação Importante sobre Acesso:** Para acessar a aplicação no navegador ou testar via Postman, utilize estritamente a URL e porta fornecidas pelo terminal (ex: `http://127.0.0.1:8000`). Digitar a URL concatenada ao localhost (como `localhost/127.0.0.1:8000`) causará um erro *403 Forbidden* de permissão de pasta no Apache.